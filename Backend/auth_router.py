...
async def login (user: userLogin):
    ...
    data = {'email': existingUser['email'], 'firstName': existingUser['firstName'], 'lastName': existingUser['lastName']}
    token = await createToken (data=data, expires=timedelta(hours=24))
